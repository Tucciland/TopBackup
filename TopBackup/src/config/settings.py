...
class FirebirdConfig:
    ...
    database_path: str = ""
    database_path_2: str = ""  # Banco secundário (opcional)
    gbak_path: str = ""
    user: str = "SYSDBA"
    ...
    def load(cls, config_path: Optional[str] = None) -> "Settings":
        ...
        if os.path.exists(config_path):
            try:
                ...
                if 'firebird' in data:
                    fb_data = data['firebird'].copy()
                    # Compatibilidade: garante que database_path_2 existe
                    if 'database_path_2' not in fb_data:
                        fb_data['database_path_2'] = ""
                    settings.firebird = FirebirdConfig(**fb_data)
