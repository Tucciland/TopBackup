...
class MySQLClient:
    ...
    def sync_empresa(self, empresa: Empresa) -> int:
        """
        Sincroniza empresa (insert ou update).

        Returns:
            ID da empresa no MySQL (0 se falhar)
        """
        ...
        if existing:
            ...
            result = self.insert_empresa(empresa)
            return result if result else 0

    def get_empresa_id_by_cnpj(self, cnpj: str) -> int:
        """
        Obtém ID da empresa pelo CNPJ.

        Args:
            cnpj: CNPJ da empresa

        Returns:
            ID da empresa ou 0 se não encontrada
        """
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT ID FROM EMPRESA WHERE CNPJ = %s", (cnpj,))
                row = cursor.fetchone()
                return row[0] if row else 0
        except Exception as e:
            self.logger.error(f"Erro ao buscar empresa por CNPJ: {e}")
            return 0
    ...
    def update_log_backup(self, log: LogBackup, max_retries: int = 3) -> bool:
        """
        Atualiza log de backup com retry automático.

        Importante: Este método DEVE atualizar o status do backup corretamente.
        Se falhar, o backup ficará com status 'E' (executando) para sempre.
        """
        if not log.id:
            self.logger.error("update_log_backup: log.id é None, não é possível atualizar")
            return False
            ...
            try:
                with self.get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        self.logger.debug(f"Log de backup ID={log.id} atualizado para STATUS={log.status}")
                        return True
                    else:
                        self.logger.warning(f"update_log_backup: nenhuma linha afetada para ID={log.id}")
                        return False

            except Exception as e:
                last_error = e
                self.logger.warning(f"update_log_backup tentativa {attempt + 1}/{max_retries} falhou: {e}")
                if attempt < max_retries - 1:
                    import time
                    time.sleep(1)  # Espera 1 segundo antes de tentar novamente

        self.logger.error(f"Erro ao atualizar log de backup após {max_retries} tentativas: {last_error}")
        return False
    ...
    def cleanup_orphan_backups(self, id_empresa: int, timeout_hours: int = 2) -> int:
        """
        Marca backups órfãos como falha.

        Backups órfãos são registros com STATUS='E' (executando) que ficaram
        "travados" por mais tempo que o timeout, indicando que o app crashou
        ou o computador foi desligado durante o backup.

        Args:
            id_empresa: ID da empresa
            timeout_hours: Horas de timeout (padrão 2 horas)

        Returns:
            Número de backups marcados como falha
        """
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                ...
                sql = """
                    ...
                    SET
                        STATUS = 'F',
                        DATA_FIM = NOW(),
                        MENSAGEM_ERRO = CONCAT(
                            'Backup interrompido (app fechado ou computador desligado). ',
                            'Iniciado há ',
                            TIMESTAMPDIFF(HOUR, DATA_INICIO, NOW()),
                            ' horas.'
                        )
                    WHERE ID_EMPRESA = %s
                    AND STATUS = 'E'
                    AND DATA_INICIO < DATE_SUB(NOW(), INTERVAL %s HOUR)
                """
                cursor.execute(sql, (id_empresa, timeout_hours))
                affected = cursor.rowcount
                conn.commit()

                if affected > 0:
                    self.logger.info(
                        f"Limpeza de backups órfãos: {affected} backup(s) marcado(s) como falha"
                    )

                return affected

        except Exception as e:
            self.logger.error(f"Erro ao limpar backups órfãos: {e}")
            return 0

    def cleanup_all_orphan_backups(self, timeout_hours: int = 2) -> int:
        """
        Marca TODOS os backups órfãos de TODAS as empresas como falha.

        Útil para limpeza geral do banco (executado pelo painel de monitoramento).
        ...
        try:
            with self.get_connection() as conn:
                ...
                if affected > 0:
                    self.logger.info(
                        f"Limpeza global de backups órfãos: {affected} backup(s) marcado(s) como falha"
                    )

                return affected

        except Exception as e:
            self.logger.error(f"Erro ao limpar backups órfãos globalmente: {e}")
            return 0
