...
class MainWindow(ctk.CTk):
    ...
    def __init__(self, controller: AppController, settings: Settings):
        ...
        # Índice do banco selecionado (0 = principal, 1 = secundário)
        self._selected_db_index = 0
    ...
    def _create_widgets(self):
        ...
        # Seletor de banco (se houver mais de um)
        self._create_db_selector()
    ...
    def _create_db_selector(self):
        """Cria seletor de banco de dados"""
        databases = self.controller.get_configured_databases()

        # Frame do seletor
        self.db_selector_frame = ctk.CTkFrame(self.main_frame, fg_color="transparent")

        # Só mostra se houver mais de um banco
        if len(databases) > 1:
            self.db_selector_frame.pack(fill="x", pady=(0, 10))
            ...
            self._db_options = []
            for db in databases:
                empresa = db['empresa'] or 'Sem empresa'
                if len(empresa) > 30:
                    empresa = empresa[:27] + "..."
                option = f"{db['nome']} - {empresa}"
                self._db_options.append(option)

            self.db_selector_var = ctk.StringVar(value=self._db_options[0])

            self.db_selector = ctk.CTkOptionMenu(
                self.db_selector_frame,
                values=self._db_options,
                variable=self.db_selector_var,
            ...
            # Indicador de quantidade de bancos
            ctk.CTkLabel(
                self.db_selector_frame,
                text=f"({len(databases)} bancos configurados)",
                font=ctk.CTkFont(size=11),
                text_color="gray"
            ).pack(side="left", padx=10)

    def _on_db_selected(self, selection: str):
        """Callback quando banco é selecionado"""
        try:
            self._selected_db_index = self._db_options.index(selection)
        except ValueError:
            self._selected_db_index = 0
    ...
    def _create_logs_area(self):
        ...
        # Título
        self.logs_title_label = ctk.CTkLabel(
            logs_frame,
            text="Últimos Backups",
            font=ctk.CTkFont(size=14, weight="bold")
        )
        self.logs_title_label.pack(anchor="w", padx=10, pady=5)
    ...
    def _update_status(self):
        """Atualiza informações de status do banco selecionado"""
        status = self.controller.get_status_for_db(self._selected_db_index)

        if not status:
            # Fallback para status geral se não houver status específico
        ...
        # Atualiza título dos logs
        databases = self.controller.get_configured_databases()
        if len(databases) > 1 and self._selected_db_index < len(databases):
            db_name = databases[self._selected_db_index]['nome']
            self.logs_title_label.configure(text=f"Últimos Backups - {db_name}")
        else:
            self.logs_title_label.configure(text="Últimos Backups")
    ...
    def _update_logs_display(self):
        """Atualiza exibição de logs do banco selecionado"""
        logs = self.controller.get_backup_logs_for_db(self._selected_db_index, limit=10)

        self.logs_text.configure(state="normal")
        self.logs_text.delete("1.0", "end")

        if not logs:
            self.logs_text.insert("end", "Nenhum backup registrado para este banco.\n")
        else:
            for log in logs:
    ...
    def _on_backup_click(self):
        """Inicia backup manual do banco selecionado"""
        self.backup_btn.configure(state="disabled")

        db_index = self._selected_db_index

        def run_backup():
            result = self.controller.execute_backup_manual_for_db(db_index)
    ...
    def _on_settings_saved(self):
        ...
        # Atualiza seletor de banco (pode ter mudado)
        self._refresh_db_selector()
        # Atualiza interface
        self._update_status()

    def _refresh_db_selector(self):
        """Atualiza o seletor de banco"""
        databases = self.controller.get_configured_databases()

        # Remove o seletor atual
        for widget in self.db_selector_frame.winfo_children():
            widget.destroy()
        ...
        if len(databases) > 1:
            ...
            if self._selected_db_index >= len(databases):
                self._selected_db_index = 0

            self.db_selector_var = ctk.StringVar(value=self._db_options[self._selected_db_index])
            ...
            self.db_selector_frame.pack_forget()
            self._selected_db_index = 0
    ...
    def _on_agendas_click(self):
        """Abre visualizador de agendas do banco selecionado"""
        agendas = self.controller.get_agendas_for_db(self._selected_db_index)
        if agendas:
            dialog = AgendaListDialog(self, agendas)
            dialog.grab_set()
        else:
            show_warning(self, "Agendas", "Nenhuma agenda de backup encontrada para este banco")
