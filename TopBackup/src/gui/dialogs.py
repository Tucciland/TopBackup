    ...
    def _clear_log_file(self):
        ...
        if messagebox.askyesno(
            ...
            if self.logger.clear_logs():
                self._load_logs()
                messagebox.showinfo("Sucesso", "Logs limpos com sucesso.", parent=self)
            else:
                messagebox.showerror("Erro", "Erro ao limpar logs.", parent=self)
...
class SettingsDialog(ctk.CTkToplevel):
    ...
    def _create_connections_tab(self):
        ...
        # Banco de dados
        ctk.CTkLabel(fb_inner, text="Banco Principal:").pack(anchor="w", pady=(5, 0))
        ...
        # Banco secundário
        ctk.CTkLabel(fb_inner, text="Banco Secundário (Opcional):").pack(anchor="w", pady=(5, 0))
        fb_db2_frame = ctk.CTkFrame(fb_inner, fg_color="transparent")
        fb_db2_frame.pack(anchor="w", fill="x")
        self.fb_db_2_entry = ctk.CTkEntry(fb_db2_frame, width=300)
        self.fb_db_2_entry.pack(side="left")
        ctk.CTkButton(
            fb_db2_frame,
            text="...",
            width=40,
    ...
    def _browse_database_2(self):
        """Abre diálogo para selecionar banco de dados secundário"""
        path = filedialog.askopenfilename(
            title="Selecione o banco de dados secundário",
            filetypes=[("Firebird Database", "*.fdb *.gdb"), ("Todos", "*.*")]
        )
        if path:
            self.fb_db_2_entry.delete(0, "end")
            self.fb_db_2_entry.insert(0, path)
