...
class BackupScheduler:
    ...
    def __init__(self, settings: Settings):
        ...
        # Callbacks
        # backup_callback agora recebe agenda opcional: Callable[[Optional[AgendaBackup]], None]
        self._backup_callback: Optional[Callable[[Optional[AgendaBackup]], None]] = None
        self._sync_callback: Optional[Callable] = None
        self._update_callback: Optional[Callable] = None
        self._cleanup_logs_callback: Optional[Callable] = None
    ...
    def set_cleanup_logs_callback(self, callback: Callable):
        """Define callback para limpeza de logs"""
        self._cleanup_logs_callback = callback
    ...
    def configure_from_agendas(self, agendas: List[AgendaBackup]):
        ...
        for i, agenda in enumerate(agendas):
            ...
            # Cria função específica para esta agenda usando partial
            job_func = partial(self._execute_backup_for_agenda, agenda)

            # Nome do job inclui nome do banco para identificação
            banco_nome = os.path.basename(agenda.banco_origem) if agenda.banco_origem else "default"
            ...
            self.scheduler.add_job(
                ...
                trigger=trigger,
                id=job_id,
                name=f'Backup {banco_nome} {hora:02d}:{minuto:02d}',
                replace_existing=True
            )

            self.logger.info(f"Backup agendado: {hora:02d}:{minuto:02d} ({dias_cron}) - Banco: {banco_nome}")
    ...
    def configure_system_jobs(self):
        """Configura jobs do sistema (sync, update, cleanup)"""
        ...
        # Job de verificação de updates (a cada 10min)
        ...
        # Job de limpeza de logs (diário às 3:00)
        if self._cleanup_logs_callback:
            self.scheduler.add_job(
                func=self._cleanup_logs_callback,
    ...
    def _execute_backup_job(self):
        """Executa job de backup (sem agenda específica)"""
        if self._backup_callback:
            try:
    ...
    def _execute_backup_for_agenda(self, agenda: AgendaBackup):
        """Executa job de backup para uma agenda específica"""
        if self._backup_callback:
            try:
                self._backup_callback(agenda)
            except Exception as e:
                self.logger.error(f"Erro no backup da agenda {agenda.id}: {e}")
        self._update_next_backup()
